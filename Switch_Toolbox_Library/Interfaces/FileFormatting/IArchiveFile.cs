    ...
    public class ArchiveFileInfo : INode
    {
        ...
        public void SaveFileFormat()
        {
            if (FileFormat != null && FileFormat.CanSave)
            {
                if (FileDataStream != null)
                {
                    Console.WriteLine($"更新文件数据流" + (FileDataStream is FileStream));
            ...
            var ToolStrips = new ToolStripItem[]
{
                    ...
                    new STToolStripItem("提取全部", ExtractAllAction),
                    new STToolStripSeparator(),
                    ...
                    new STToolStripItem("清楚文件", ClearAction) { Enabled = ArchiveFile.CanDeleteFiles},
            };
        ...
        private void RepackAction(object sender, EventArgs args)
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int i = 0; i < ProccessedFiles.Count; i++)
                {
                    ...
                    progressBar.Task = $"打包中 {ProccessedFiles[i].Item1}";
                    progressBar.Refresh();
    ...
    public class ArchiveFileWrapper : ArchiveBase, IContextMenuNode
    {
        ...
        public ToolStripItem[] GetContextMenuItems()
        {
            return new ToolStripItem[]
            {
                ...
                new STToolStripSeparator(),
                new STToolStipMenuItem("打开文本编辑器", null, OpenTextEditorAction, Keys.Control | Keys.T),
                new STToolStripSeparator(),
        ...
        private void DeleteAction(object sender, EventArgs args)
        {
            DialogResult result = MessageBox.Show($"您确定要删除吗{Text}?这是无法挽回的!", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
