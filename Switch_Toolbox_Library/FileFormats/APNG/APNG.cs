        ...
        public static uint ParseUint(byte[] buffer, int uintLengthInBytes, ref int offset)
        {
            ...
            if (uintLengthInBytes > sizeof(uint))
                throw new ArgumentException(
                    String.Format("函数只能用于解析缓冲区中最多{0}个字节",
                    sizeof(uint)));
            if (buffer.Length - offset < uintLengthInBytes)
                throw new ArgumentException(
                    String.Format("缓冲区不够长，无法在偏移量 {1 处提取{0}个字节",
                    sizeof(uint), offset));
            int i, j;
            ...
            if (signature.Length == expectedSignature.Length)
            {
                for (int i = 0; i < expectedSignature.Length; i++)
                {
                    ...
                    if (expectedSignature[i] != signature[i])
                        throw new ApplicationException("APNG签名未找到.");
                }
            }
            else
                // Invalid signature
    ...
    internal class APNGChunk
    {
        ...
        public APNGChunk(APNGChunk chunk, String expectedType)
        {
            ...
            if (ChunkType != expectedType)
                throw new ArgumentException(
                    String.Format("指定的块类型不是预期的{0}", expectedType));
        ...
        public void Read(Stream stream)
        {
            ...
            if (ChunkCRC != calculatedCRC)
            {
                ...
                sb.AppendLine(String.Format("APNG Chunk CRC不匹配.  Chunk CRC = {0}, Calculated CRC = {1}.",
                    ChunkCRC, calculatedCRC));
                sb.AppendLine(String.Format("这是在解析流中位置为{0}（0x {1:X} 的块时发生的情况.",
        ...
        public Bitmap ToBitmap(int index)
        {
            ...
            if (index > NumEmbeddedPNG)
                throw new ArgumentException(String.Format(
                    "嵌入式PNG索引必须介于0和{0}之间", NumEmbeddedPNG - 1));
            // Create the bitmap
