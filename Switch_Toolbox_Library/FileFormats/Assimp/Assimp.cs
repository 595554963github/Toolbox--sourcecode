    ...
    public class AssimpData
    {
        ...
        public bool LoadFile(string FileName)
        {
            try
            {
                ...
                if (settings.ShowDialog() == DialogResult.OK)
                {
                    ...
                    STConsole.WriteLine($"加载文件{FileName}", Color.FromArgb(0, 255, 0));
            ...
            {
                if (e.ToString().Contains("从路径加载非托管库时出错"))
                {
                    MessageBox.Show($"加载assimp失败!确保程序旁边有Assimp32.dll!");
                }
                else
        ...
        public void processNode()
        {
            ...
            if (scene.RootNode != null)
            {
                ...
                Console.WriteLine($"构建节点网格数量" + scene.MeshCount);
            }
            else
            {
                STConsole.WriteLine("未找到根节点!将默认使用单位矩阵!", Color.Red);
        ...
        public static Animations.Animation CreateGenericAnimation(Assimp.Animation animation)
        {
            ...
            if (animation.HasNodeAnimations)
            {
                ...
                for (int i = 0; i < _channels.Length; i++)
                {
                    ...
                    STConsole.WriteLine($"创建骨骼动画{boneAnim.Text} ");
        ...
        private STGenericMatTexture CreateTextureSlot(TextureSlot tex, TextureType type)
        {
            ...
            STConsole.WriteLine($"获取assimp纹理槽{matTex.Name}类型{matTex.Type}");
            ...
            short SmoothIndex, short RigidIndex, bool IsRoot, ref Assimp.Matrix4x4 rootTransform)
        {
            ...
            {
                STConsole.WriteLine($"为骨找到重复的节点名称{node.Name}!", Color.Red);
            }
