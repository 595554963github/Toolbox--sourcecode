    ...
    public class DAE
    {
            ...
            List<STGenericTexture> Textures, STSkeleton skeleton = null, List<int> NodeArray = null)
        {
            ...
            using (ColladaWriter writer = new ColladaWriter(FileName, settings))
            {
                ...
                if (Materials.Count > 0)
                {
                    ...
                    for (int i = 0; i < Textures?.Count; i++)
                    {
                        ...
                        if (settings.ExportTextures)
                        {
                            ...
                            progressBar.Task = $"正在导出模型{Textures[i].Text}";
                ...
                if (skeleton != null)
                {
                    ...
                    foreach (var bone in skeleton.bones)
                    {
                        ...
                        {
                            Console.WriteLine("跳过" + bone.Text);
                            continue;
                        }
                ...
                {
                    progressBar.Task = $"正在导出网格{mesh.Text}";
            ...
            if (!settings.SuppressConfirmDialog)
                System.Windows.Forms.MessageBox.Show($"导出{FileName}成功!");
        }
