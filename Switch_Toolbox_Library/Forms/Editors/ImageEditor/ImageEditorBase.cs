    ...
    public partial class ImageEditorBase : UserControl
    {
        ...
        private void UpdateLabel()
        {
            ...
            if (!Runtime.ImageEditor.DisplayAlpha)
                noteifcationLbl.Text += "注意编辑器中隐藏的alpha通道!";
        }
        ...
        public void UpdateMipDisplay()
        {
            if (HasBeenEdited)
            {
                var message = MessageBox.Show("此纹理已编辑!您要应用所做的更改吗?", "图片编辑器", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
        ...
        private void OnFileWatcherChanged(object sender, FileSystemEventArgs e)
        {
            ...
            var Result = MessageBox.Show("纹理已在外部程序中修改!您想应用编辑吗?", "图片编辑器",
