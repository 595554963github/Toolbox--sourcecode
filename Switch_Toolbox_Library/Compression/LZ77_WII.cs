    ...
    public class LZ77_WII
    {
            ...
            while (curr_size < decomp_size)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if (flag)
                    {
                        ...
                        catch (EndOfStreamException ex) { throw new InvalidDataException("数据不完整", ex); }
                        n = b >> 4;
                        disp = (b & 0x0F) << 8;
                        ...
                        if (disp > curr_size) { throw new InvalidDataException("不能回溯超过已写入的量"); }
                        for (int j = 0; j < n; j++)
                        {
                    ...
                    else
                    {
                        ...
                        {
                            Console.Error.WriteLine("数据不完整, " + ex);
                            break; 
                        }
        ...
        public static byte[] Decompress(byte[] input, bool useMagic = true)
        {
            ...
            using (var reader = new FileReader(new MemoryStream(input), true))
            {
                ...
                if (temp[4] != 0x10)
                { reader.Dispose(); throw new Exception("不支持的压缩类型!"); }
