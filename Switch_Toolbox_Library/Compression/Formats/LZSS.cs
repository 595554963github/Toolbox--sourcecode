...
namespace Toolbox.Library
{
    ...
    {
        public string[] Description { get; set; } = new string[] { "LZSS压缩" };
        ...
        public Stream Decompress(Stream stream)
        {
            ...
            if (hasMagic)
            {
                ...
                if (arcdata.Length != compressedSize + 0x10) throw new Exception("压缩尺寸不匹配");
            }
            else
            {
            ...
            if (decompressedSize != outdata.Count)
                throw new Exception(string.Format("尺寸不匹配:{0}解压后得到字节，预期{1}.\n", outdata.Count, decompressedSize));
