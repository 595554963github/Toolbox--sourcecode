...
namespace Toolbox.Library
{
    ...
    public abstract class STGenericTexture : STGenericWrapper
    {
        ...
        public uint MipCount
        {
            ...
            set
            {
                if (value == 0)
                    ...
                    throw new Exception($"mip映射计数无效! 纹理: {Text} 值: {value}");
                else
                    mipCount = value;
            }
        }
        ...
        public static byte[] DecodeBlock(byte[] data, uint Width, uint Height, TEX_FORMAT Format, byte[] paletteData, ImageParameters parameters, PALETTE_FORMAT PaletteFormat = PALETTE_FORMAT.None, PlatformSwizzle PlatformSwizzle = PlatformSwizzle.None)
        {
            ...
            if (Format <= 0) throw new Exception($"格式无效!");
            if (data.Length <= 0) throw new Exception($"数据为空!");
            if (Width <= 0) throw new Exception($"宽度大小无效{Width}!");
            if (Height <= 0) throw new Exception($"高度尺寸无效{Height}!");
        ...
        public override string ExportFilter
        {
            get
            {
                if (IsAtscFormat(Format))
                {
                    return "支持的格式|*.dds; *.astc; *.png;*.tga;*.jpg;*.tiff|" +
                                "ASTC |*.astc|" +
                ...
                {
                    return "支持的格式|*.dds; *.png;*.tga;*.jpg;*.tiff|" +
                                 "Microsoft DDS |*.dds|" +
                                 "Portable Network Graphics |*.png|" +
                                 "Joint Photographic Experts Group |*.jpg|" +
                                 "Bitmap Image |*.bmp|" +
        ...
        {
            if (bytes == null)
                throw new Exception("数据块返回为空，确保参数和图像属性正确!");
        ...
        public class Properties
        {
            ...
            [Browsable(true)]
            [ReadOnly(true)]
            [Description("图像的mip映射计数.")]
            [Category("图像信息")]
            public uint MipCount { get; set; }

            [Browsable(true)]
            [ReadOnly(true)]
            [Description("多个表面的图像数组计数.")]
            [Category("图像信息")]
            public uint ArrayCount { get; set; }

            [Browsable(true)]
            [ReadOnly(true)]
            [Description("以字节为单位的图像大小.")]
            [Category("图像信息")]
