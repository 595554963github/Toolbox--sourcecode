...
namespace FirstPlugin
{
    public class FirstPlugin : IPlugin
    {
        ...
        class MenuExt : IMenuExtension
        {
            ...
            public MenuExt()
            {
                toolsExt[0] = new STToolStripItem("超级马里奥:奥德赛");
                toolsExt[0].DropDownItems.Add(new STToolStripItem(" Kingdom选择器", OpenKingdomSelector));
                toolsExt[0].DropDownItems.Add(new STToolStripItem(" Costume选择器", OpenSelector));

                toolsExt[1] = new STToolStripItem("马里奥赛车8");
                toolsExt[1].DropDownItems.Add(new STToolStripItem("Probe Light转换器", GenerateProbeLightBounds));
            ...
            private void PokemonLoaderSwSh(object sender, EventArgs args)
            {
                ...
                {
                    var result = MessageBox.Show("请设置您的口袋妖怪剑/盾游戏路径!");
            ...
            private void OpenSelector(object sender, EventArgs args)
            {
                ...
                {
                    var result = MessageBox.Show("请设置您的马里奥:奥德赛路径!");
                    if (result == DialogResult.OK)
                    {
                        FolderSelectDialog ofd = new FolderSelectDialog();
                        if (ofd.ShowDialog() == DialogResult.OK)
