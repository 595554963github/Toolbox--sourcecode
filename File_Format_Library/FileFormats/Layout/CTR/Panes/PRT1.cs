...
namespace LayoutBXLYT.CTR
{
    public class PRT1 : PAN1, IPartPane
    {
        ...
        [DisplayName("X轴放大"), CategoryAttribute("部件")]
        public float MagnifyX { get; set; }

        [DisplayName("Y轴放大"), CategoryAttribute("部件")]
        public float MagnifyY { get; set; }

        [DisplayName("属性"), CategoryAttribute("部件")]
        public List<PartProperty> Properties { get; set; }

        private string layoutFilename;

        [DisplayName("外部Layout文件"), CategoryAttribute("部件")]
            ...
            foreach (var file in archiveFile.Files)
            {
                ...
                else if (file.FileName.Contains(LayoutFileName))
                {
                    try
                    {
                        ...
                        {
                            Console.WriteLine("找到的部分! " + file.FileName);
            ...
            if (propertyOffset != 0)
            {
                ...
                switch (signtature)
                {
                    ...
                    case "prt1":
                        ...
                        break;
                    default:
                        Console.WriteLine("未知部分! " + signtature);
                        break;
                }
            }
            if (userDataOffset != 0)
