...
namespace FirstPlugin
{
    public class XTX : TreeNodeFile, IFileFormat, IContextMenuNode, ITextureContainer
    {
        ...
        public void LoadFile(Stream data)
        {
            ...
            if (Signature != "DFvN")
                throw new Exception($"无效签名{Signature}!预期为DFvN.");
        ...
        public class BlockHeader
        {
            ...
            public void Read(FileReader reader)
            {
                ...
                string Signature = reader.ReadString(4, Encoding.ASCII);
                if (Signature != "HBvN")
                    throw new Exception($"无效签名{Signature}!预期为HBvN.");
        ...
        public class TextureInfo : STGenericTexture
        {
            ...
            public static XTXImageFormat ConvertFromGenericFormat(TEX_FORMAT Format)
            {
                switch (Format)
                {
                    ...
                    default:
                        throw new Exception($"无法转换格式{Format}");
                }
            }
