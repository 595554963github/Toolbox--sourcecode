...
namespace FirstPlugin
{
    public class BFLIM : STGenericTexture, IEditor<ImageEditorBase>, IFileFormat
    {
        ...
        public string[] Description { get; set; } = new string[] { "布局图像" };
        ...
        private void UpdateForm(ImageEditorBase form)
        {
            ...
            if (image != null)
            {
                ...
                form.ResetMenus();
                form.AddFileContextEvent("保存", Save);
                form.AddFileContextEvent("替换", Replace);
                form.LoadProperties(prop);
                form.LoadImage(this);

                Console.WriteLine("更新表单加载图像");
            }
        }
        ...
        class MenuExt : IFileMenuExtension
        {
            ...
            public MenuExt()
            {
                ...
                toolExt[0].DropDownItems.Add(new STToolStripItem("批量导出(Wii U纹理)", Export));
                newFileExt[0] = new STToolStripItem("来自BFLIM图像", CreateNew);
            }
        ...
        public class Properties
        {
            ...
            [Browsable(true)]
            [ReadOnly(true)]
            [Description("图像的深度（3D类型）.")]
            [Category("图像信息")]
            public uint Depth { get; set; }

            [Browsable(true)]
            [ReadOnly(true)]
            [Description("图像的多级渐远纹理计数.")]
            [Category("图像信息")]
            public uint MipCount { get; set; }

            [Browsable(true)]
            [ReadOnly(true)]
            [Description("用于多个表面的图像数组计数.")]
            [Category("图像信息")]
            public uint ArrayCount { get; set; }

            [Browsable(true)]
            [ReadOnly(true)]
            [Description("图像大小（以字节为单位）.")]
            [Category("图像信息")]
