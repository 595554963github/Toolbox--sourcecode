...
namespace FirstPlugin
{
    public class BNTX : TreeNodeFile, IFileFormat, IContextMenuNode, ITextureContainer
    {
        ...
        class MenuExt : IFileMenuExtension
        {
            ...
            public MenuExt()
            {
                toolExt[0] = new STToolStripItem("纹理");
                toolExt[0].DropDownItems.Add(new STToolStripItem("批量导出(BNTX)", Export));

                newFileExt[0] = new STToolStripItem("BNTX", New);
                newFromFileExt[0] = new STToolStripItem("来自BNTX图像", NewFromFile);
            }
        ...
        private static void SearchForBinaryContainerFile(FileReader reader, int Alignment, List<BNTX> Containers)
        {
            ...
            bntx.IFileInfo = new IFileInfo();
            bntx.Text = "工作表" + Containers.Count;
            ...
            Console.WriteLine("文件容器" + bntx.Text + " " + StartPos);
        ...
        public ToolStripItem[] GetContextMenuItems()
        {
            return new ToolStripItem[]
            {
                ...
                new ToolStripSeparator(),
                new ToolStripMenuItem("清理", null, Clear, Keys.Control | Keys.C),
            };
        }
        ...
        private List<TextureData> ImportTexture(List<TextureImporterSettings> settings, STCompressionMode CompressionMode)
        {
            ...
            foreach (var setting in settings)
            {
                ...
                if (setting.DataBlockOutput.Count <= 0)
                    throw new Exception("数据块为空!压缩失败!");
        ...
        {
            var result = MessageBox.Show("你确定要清除这个区域吗?这是无法撤销的!",
    ...
    public class TextureData : STGenericTexture
    {
        ...
        public override void SetImageData(Bitmap bitmap, int ArrayLevel)
        {
            ...
            {
                STErrorDialog.Show("无法混合和压缩图像" + Text, "错误", ex.ToString());
            }
        }
        ...
        public override byte[] GetImageData(int ArrayLevel = 0, int MipLevel = 0, int DepthLevel = 0)
        {
            ...
            {
                MessageBox.Show($"纹理混合失败{Text}!异常: {e}");
                return new byte[0];
            }
        }
