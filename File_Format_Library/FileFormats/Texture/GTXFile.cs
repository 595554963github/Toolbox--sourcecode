    ...
    public class GTXFile : TreeNodeFile, IFileFormat, IContextMenuNode, ITextureContainer
    {
        ...
        public ToolStripItem[] GetContextMenuItems()
        {
            return new ToolStripItem[]
            {
                ...
                new ToolStripMenuItem("导出全部", null, ExportAllAction, Keys.Control | Keys.E),
            };
        }
        ...
        public void Save(System.IO.Stream stream)
        {
            using (FileWriter writer = new FileWriter(stream, true))
            {
                ...
                else if (header.MajorVersion == 6 || header.MajorVersion == 7)
                {
                    ...
                    throw new Exception($"不支持的GTX版本{header.MajorVersion}");
        ...
        private void ReadGx2(FileReader reader)
        {
            ...
            if (header.GpuVersion != 2)
                throw new Exception($"不支持的GPU版本{header.GpuVersion}");
            ...
            while (reader.Position < reader.BaseStream.Length)
            {
                ...
                if ((uint)block.BlockType == surfBlockType)
                {
                    ...
                    if (surface.tileMode == 0 || surface.tileMode > 16)
                        throw new Exception($"无效的平铺模式{surface.tileMode}!");

                    if (surface.numMips > 14)
                        throw new Exception($"无效的Mip贴图数量{surface.numMips}!");
                ...
                else if ((uint)block.BlockType == mipBlockType)
                {
                    ...
                    Nodes.Add(new BlockDisplay(block.data) { Text = "顶点着色器头" });
                else if ((uint)block.BlockType == vertexShaderProgram)
                    Nodes.Add(new BlockDisplay(block.data) { Text = "顶点着色器程序" });
                else if ((uint)block.BlockType == pixelShaderHeader)
                    Nodes.Add(new BlockDisplay(block.data) { Text = "像素着色器头" });
                else if ((uint)block.BlockType == pixelShaderProgram)
                    Nodes.Add(new BlockDisplay(block.data) { Text = "像素着色器程序" });
                else if ((uint)block.BlockType == geometryShaderHeader)
                    Nodes.Add(new BlockDisplay(block.data) { Text = "几何着色器头" });
                else if ((uint)block.BlockType == geometryShaderProgram)
                    Nodes.Add(new BlockDisplay(block.data) { Text = "几何着色器程序" });
                else if (!BlockIsEmpty)
                    Nodes.Add(new BlockDisplay(block.data) { Text = $"块类型{block.BlockType.ToString("X")}" });
            }
        ...
        public class GTXHeader
        {
            ...
            public void Read(FileReader reader)
            {
                ...
                if (Signature != Magic)
                    throw new Exception($"无效签名{Signature}!预期为Gfx2.");
        ...
        public class GTXDataBlock
        {
            ...
            public void Read(FileReader reader)
            {
                ...
                string Signature = reader.ReadString(4, Encoding.ASCII);
                if (Signature != Magic)
                    throw new Exception($"无效签名{Signature}!预期为BLK.");
