...
namespace FirstPlugin
{
    public class GFBMDL : TreeNodeFile, IContextMenuNode, IFileFormat
    {
        ...
        public string[] Description { get; set; } = new string[] { "Graphic模型" };
            ...
            STSkeleton skeleton)
        {
           ...
           //     bone.RigidCheck = new BoneRigidData();
            ...
            if (unmappedBones.Count > 0)
                STErrorDialog.Show($"{unmappedBones.Count}骨骼在骨骼集合中不存在，且未被映射!", 
                    "GFBMDL导入器", string.Join("\n", unmappedBones.ToArray()));
