...
namespace FirstPlugin.LuigisMansion3
{
    ...
    public class LM3_DICT : TreeNodeFile, IFileFormat, IContextMenuNode, ILeaveOpenOnLoad
    {
        ...
        public string[] Description { get; set; } = new string[] { "路易吉的豪宅3 字典" };
                    ...
                    switch (chunk.ChunkType)
                    {
                        ...
                        case SubDataType.TextureHeader:
                            ...
                            using (var textureReader = new FileReader(chunkEntry.FileData, true))
                            {
                                ...
                                if (DebugMode)
                                    currentTexture.Text = $"纹理{ImageHeaderIndex} {currentTexture.Unknown} {currentTexture.Unknown2} {currentTexture.Unknown3.ToString("X")}";
                                else
                                    currentTexture.Text = $"纹理{currentTexture.ID2.ToString("X")}";
         ...
         /*       OpenFileDialog ofd = new OpenFileDialog();
            ...
            public System.IO.Stream GetData()
            {
                ...
                if (System.IO.File.Exists(DataFile))
                {
                    ...
                    Console.WriteLine("路径不存在! " + DataFile);
