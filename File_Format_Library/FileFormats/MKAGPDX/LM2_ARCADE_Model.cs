...
namespace FirstPlugin
{
    public class LM2_ARCADE_Model : TreeNodeFile, IFileFormat
    {
        ...
        public string[] Description { get; set; } = new string[] { "路易吉洋楼2模型" };
        ...
        class MenuExt : IFileMenuExtension
        {
            ...
            public MenuExt()
            {
                ...
                toolExt[0].DropDownItems.Add(new STToolStripItem("批量导出(LM2A .bin)", BatchExport));
                toolExt[0].DropDownItems.Add(new STToolStripItem("组合批量导出(LM2A .bin)", BatchExportCombined));
            }
            ...
            public void BatchExportCombined(object sender, EventArgs args)
            {
                ...
                ofd.Filter = "支持的格式|*.bin";
                if (ofd.ShowDialog() != DialogResult.OK) return;

                SaveFileDialog sfd = new SaveFileDialog();
                sfd.Filter = "支持的格式|*.dae";
        ...
        public class Header
        {
            ...
            public void Read(FileReader reader, LM2_ARCADE_Model root)
            {
                ...
                {
                    throw new Exception("尚不支持动画文件!");
                }
                else
                {
                    throw new Exception("找到未知类型! " + Type);
                }
