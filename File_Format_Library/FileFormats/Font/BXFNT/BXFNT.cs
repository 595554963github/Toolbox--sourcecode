...
namespace FirstPlugin
{
    public class BXFNT : IFileFormat, IEditor<BffntEditor>, IConvertableTextFormat
    {
        ...
        public string[] Description { get; set; } = new string[] { "Cafe字体", "CTR字体", "Revolution字体", "Revolution档案字体" };
    ...
    public class FFNT
    {
        ...
        public void Read(FileReader reader)
        {
            ...
            if (Signature != "FFNT" && Signature != "CFNT" && Signature != "RFNT" && Signature != "TNFR" && Signature != "RFNA")
                throw new Exception($"签名无效{Signature}! 预期FFNT or CFNT or RFNT.");
            ...
            while (!reader.EndOfStream)
            {
                ...
                switch (BlockSignature)
                {
                    ...
                    case "HTGL":
                        break;
                    default:
                        throw new Exception("发现未知块! " + BlockSignature);
                }
