    ...
    public class GFPAK : TreeNodeFile, IArchiveFile, IFileFormat
    {
        ...
        public void Load(System.IO.Stream stream)
        {
            ...
            foreach (var file in files)
            {
                ...
                if (!folders.ContainsKey(folderName))
                {
                    ...
                    if (folderName == "纹理")
                        folder = new TextureFolder(this, "纹理");
                    if (folderName == "模型")
                        folder = new ModelFolder("模型");
                    if (folderName == "动画")
                        folder = new AnimationFolder("动画");
        ...
        public class TextureFolder : TreeNodeCustom, IContextMenuNode
        {
            ...
            public virtual ToolStripItem[] GetContextMenuItems()
            {
                List<ToolStripItem> Items = new List<ToolStripItem>();
                Items.Add(new ToolStripMenuItem("导出全部", null, ExportAllAction, Keys.Control | Keys.E));
                Items.Add(new ToolStripMenuItem("替换纹理(从文件夹)", null, ReplaceAllAction, Keys.Control | Keys.R));
                return Items.ToArray();
            }
        ...
        public void Save(System.IO.Stream stream)
        {
            ...
            {
                MessageBox.Show("在可执行文件夹中必须有oo2core_6_win64.dll.");
                return;
            }
        ...
        public void Read(FileReader reader)
        {
            ...
            if (Signature != "GFLXPACK")
                throw new Exception($"字符串标记无效{Signature}!预期字符串为GFLXPACK.");
        ...
        public class FileEntry : ArchiveFileInfo
        {
            ...
            public void Read(FileReader reader)
            {
                ...
                if (Type == CompressionType.Oodle)
                {
                    ...
                    {
                        MessageBox.Show("'oo2core_6_win64.dll'在可执行文件夹中找不到!用户必须提供自己的副本!");
