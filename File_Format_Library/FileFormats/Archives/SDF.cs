...
namespace FirstPlugin
{
    public class SDF : IArchiveFile, IFileFormat
    {
        ...
        public string[] Description { get; set; } = new string[] { "Snowdrop引擎数据表内容" };
        ...
        public void Load(System.IO.Stream stream)
        {
            using (var reader = new FileReader(stream))
            {
                ...
                {
                    MessageBox.Show($"文件扩展名使用了大量节点." +
                        $"这将被过滤掉以防止缓慢启动. {ExtsToString(FilteredExtensions.ToArray())}");
                }
                //Remove unused data
                startId = null;
        ...
        public void ParseNames(FileReader reader, string Name = "")
        {
            ...
            if (ch == 0)
                throw new Exception("文件树中出现意外字节");
