    ...
    public class BezelEngineArchive : IFileData
    {
        ...
        void IFileData.Save(FileSaver saver)
        {
            ...
            if (VersionMajor2 >= 5)
            {
                ...
                saver.SaveRelocateEntryToSection(saver.Position, 1, 1, 0, 1, "Asst偏移数组");
                saver.SaveFileAsstPointer();
                saver.SaveRelocateEntryToSection(saver.Position, 1, 1, 0, 1, "DIC"); 
                saver.SaveFileDictionaryPointer();
                saver.SaveRelocateEntryToSection(saver.Position, 1, 1, 0, 1, "文件名");
                saver.SaveString(Name);
                saver.SaveRelocateEntryToSection(saver.Position, 1, 1, 0, 1, "压缩名称");
                saver.SaveString(CompressionName);
                saver.SaveRelocateEntryToSection(saver.Position, 1, 1, 0, 1, "引用偏移");
