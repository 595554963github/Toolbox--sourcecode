...
namespace FirstPlugin
{
    public class BFSAR : TreeNodeFile, IFileFormat
    {
        ...
        public string[] Description { get; set; } = new string[] { "Cafe音频档案" };
        ...
        public class FSAR
        {
            ...
            public void Read(FileReader reader)
            {
                ...
                if (Signature != "FSAR")
                    throw new Exception($"无效的字符串{Signature}!预期为FSAR.");
        ...
        public class INFO
        {
            ...
            public void Read(FileReader reader)
            {
                string Signature = reader.ReadString(4, Encoding.ASCII);
                if (Signature != "INFO")
                    throw new Exception($"无效的字符串{Signature}!预期为INFO.");
        ...
        public class FILE
        {
            ...
            public void Read(FileReader reader, INFO INFO)
            {
                string Signature = reader.ReadString(4, Encoding.ASCII);
                if (Signature != "FILE")
                    throw new Exception($"无效的字符串{Signature}!预期为FILE.");
        ...
        public class STRG
        {
            ...
            public void Read(FileReader reader)
            {
                string Signature = reader.ReadString(4, Encoding.ASCII);
                if (Signature != "STRG")
                    throw new Exception($"无效的字符串{Signature}!预期为STRG.");
