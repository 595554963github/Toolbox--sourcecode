...
namespace DKCTF
{
    public class CModel : TreeNodeFile, IFileFormat, IExportableModel
    {
        ...
        public void LoadTextures(Dictionary<string, FileEntry> Textures)
        {
            ...
            foreach (var mat in ModelData.Materials)
            {
                foreach (var texMap in mat.Textures)
                {
                    ...
                    {
                        Console.WriteLine($"pak文件中不存在纹理! {mat.Name} {texMap.Key} {guid}");
                        continue;
                    }
        ...
        TreeNode texFolder = new STTextureFolder("纹理");
        TreeNode skeletonFolder = new STTextureFolder("骨架");
        TreeNode shaderFolder = new STTextureFolder("材质");
