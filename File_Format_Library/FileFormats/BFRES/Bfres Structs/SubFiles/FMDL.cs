...
namespace Bfres.Structs
{
    public class FMDL : STGenericModel, IContextMenuNode
    {
        ...
        {
            DialogResult dialogResult = MessageBox.Show("您确定要删除此模型吗?这无法撤消!", "", MessageBoxButtons.YesNo);
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (var shape in shapes)
                {
                    ...
                    if ((!shape.vertexAttributes.Any(x => x.Name == $"_u{dest}")))
                    {
                        ...
                        {
                            DialogResult dialogResult = MessageBox.Show($"({dest}一些对象缺少要复制到的目标纹理坐标（UV）通道.要创建一个吗?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                            if (dialogResult == DialogResult.Yes)
                            {
                                CanCreateUV = true;
                                ShownDialog = true;
        ...
        private void CalcTansBitansAllShapes()
        {
            ...
            foreach (FSHP shp in shapes)
            {
                ...
                if (!HasBiTans)
                {
                    DialogResult dialogResult2 = MessageBox.Show("网格没有副切线,你想要创建它们吗?（会使文件大小变大）", "", MessageBoxButtons.YesNo);
                ...
                if (!HasTans)
                {
                    DialogResult dialogResult = MessageBox.Show("网格没有切线.你想要创建它们吗?（会使文件大小变大）", "", MessageBoxButtons.YesNo);
                ...
                case ".csv":
                    ...
                    if (csvsettings.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        foreach (STGenericObject obj in csvModel.objects)
                        {
                            ...
                            {
                                STConsole.WriteLine($"在网格{obj.ObjectName}上发现0个顶点!将添加一个空网格以防止出现问题.", 0);
                ...
                default:
                    ...
                    if (settings.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (UseMats)
                        {
                            ...
                            {
                                progressBar.Task = $"生成材质{ mat.Text } {curMat} / {ImportedMaterials.Count}";
                        ...
                        Console.WriteLine("处理数据,对象计数" + ImportedObjects.Count);
