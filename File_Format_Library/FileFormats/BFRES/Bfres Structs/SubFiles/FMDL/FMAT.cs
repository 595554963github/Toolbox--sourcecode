...
namespace Bfres.Structs
{
    public class FMATFolder : TreeNodeCustom
    {
        public FMATFolder()
        {
            ...
            ContextMenuStrip.Items.Add(new ToolStripMenuItem("（从文件夹）替换)", null, ReplaceBatchAction, Keys.Control | Keys.R));
        }
    ...
    public class FMAT : STGenericMaterial, IContextMenuNode
    {
        ...
        public void Delete()
        {
            ...
            {
                MessageBox.Show("如果有任何物体存在，那么单一物质必须存在!", "删除材质",
            ...
            if (MappedNames != "")
            {
                var result = STOptionsDialog.Show("形状映射到此材料,您确定要删除此内容吗? (将默认为第一种材料)",
                    "删除材质", MappedNames);
    ...
    public class BfresShaderParam
    {
        ...
        public void ReadValue(FileReader reader, int Size)
        {
            switch (Type)
            {
                ...
                // Invalid
                default:
                    throw new ArgumentException($"无效{nameof(ShaderParamType)} {Type}.",
               nameof(Type));
            }
        }
