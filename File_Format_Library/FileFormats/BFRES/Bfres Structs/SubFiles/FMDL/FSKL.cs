        ...
        public class fsklNode : STGenericWrapper, IContextMenuNode
        {
            ...
            public ToolStripItem[] GetContextMenuItems()
            {
                return new ToolStripItem[]
                {
                    ...
                    new ToolStripMenuItem("(从骨架)替换匹配骨骼", null, ReplaceMatchingFileAction, Keys.Control | Keys.S),
                    new ToolStripMenuItem("(从文件夹)替换匹配骨骼", null, ReplaceMatchingFolderAction, Keys.Control | Keys.F),
                    new ToolStripSeparator(),
            ...
            {
                DialogResult dialogResult = MessageBox.Show("您确定要移除所有骨头吗?这无法撤销!", "", MessageBoxButtons.YesNo);
    ...
    public class BfresBone : STBone, IContextMenuNode
    {
        ...
        public void RenameBone(string Name)
        {
            if (((FSKL)skeletonParent).node.SkeletonU != null)
            {
                ...
                {
                    MessageBox.Show("具有相同名称的骨头退出!确保使用唯一的名称!", 
                        "骨架重命名", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public void Delete()
        {
            ...
            if (MappedNames != "")
            {
                var result = STOptionsDialog.Show("形状映射到这块骨头上.您确定要删除此内容吗? (会默认第一块骨头)",
                    "骨骼删除", MappedNames);
