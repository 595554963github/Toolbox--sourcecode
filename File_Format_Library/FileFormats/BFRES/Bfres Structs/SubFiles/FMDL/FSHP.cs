...
namespace Bfres.Structs
{
    public class FSHPFolder : TreeNodeCustom, IContextMenuNode
    {
        ...
        public ToolStripItem[] GetContextMenuItems()
        {
            ...
            Items.Add(new ToolStripMenuItem("新的空对象", null, CreateEmpty, Keys.Control | Keys.N));
            Items.Add(new ToolStripSeparator());
            Items.Add(new ToolStripMenuItem("导入所有对象", null, ExportAll, Keys.Control | Keys.A));
            Items.Add(new ToolStripMenuItem("清理所有对象", null, Clear, Keys.Control | Keys.C));
        ...
        {
            DialogResult dialogResult = MessageBox.Show("您确定要删除所有对象吗?这是无法挽回的!", "", MessageBoxButtons.YesNo);
        ...
        private void Import(object sender, EventArgs args)
        {
            ...
            ofd.Filter = "支持的格式|*.bfobj;*.fbx;*.dae; *.obj;*.csv;|" +
    ...
    public class FSHP : STGenericObject, IContextMenuNode
    {
        ...
        public ToolStripItem[] GetContextMenuItems()
        {
            ...
            Items.Add(new ToolStripMenuItem("更换（静态）", null, Replace, Keys.Control | Keys.R));
            Items.Add(new ToolStripSeparator());
            Items.Add(new ToolStripMenuItem("重命名", null, Rename, Keys.Control | Keys.N));
            Items.Add(new ToolStripSeparator());

            ToolStripMenuItem lodMenu = new ToolStripMenuItem("细节级别");
            lodMenu.DropDownItems.Add(new ToolStripMenuItem("清除细节级别网格", null, ClearLODMeshes));
            lodMenu.DropDownItems.Add(new ToolStripMenuItem("添加虚拟细节级别网格", null, GenerateDummyLODMeshesAction));
            Items.Add(lodMenu);
            ...
            ToolStripMenuItem uvMenu = new ToolStripMenuItem("纹理坐标");
            ...
            Items.Add(new ToolStripMenuItem("用顶点填充切线空间", null, FillBitangentsAction, Keys.Control | Keys.B));
        ...
        {
            if (Parent == null)
                STErrorDialog.Show($"错误! Shape {Text}没有父节点!", "获取素材", "");
        ...
        private void CopyUVChannelAction(object sender, EventArgs args)
        {
            ...
            {
                DialogResult dialogResult = MessageBox.Show("此模型没有第二个uv通道可以复制到.创建一个?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dialogResult == DialogResult.Yes)
                {
            ...
            {
                throw new Exception("只找到一个纹理坐标.无法应用目标纹理坐标");
            }
        }
        ...
        {
            DialogResult dialogResult = MessageBox.Show("您确定要删除此对象吗?这是无法挽回的!", "", MessageBoxButtons.YesNo);
        ...
        public void CheckVertexColors()
        {
            if (!vertexAttributes.Any(x => x.Name == "_c0"))
            {
                DialogResult dialogResult2 = MessageBox.Show($"网格{Text}没有顶点颜色.你想创建它们吗? (会使文件体积更大)", "", MessageBoxButtons.YesNo);
        ...
        public void ApplyImportSettings(BfresModelImportSettings settings, FMAT mat)
        {
            ...
            if (settings.EnableTangents)
            {
                ...
                {
                    STErrorDialog.Show($"无法为网格生成切线{Text}", "切线计算", ex.ToString());
                }
            }
        ...
        private void CalcTansBitans(object sender, EventArgs args)
        {
            ...
            if (!HasAttributeUV0())
            {
                MessageBox.Show($"错误!{Text}没有纹理坐标!", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (!HasBiTans)
            {
                DialogResult dialogResult2 = MessageBox.Show($"网格{Text}没有副切线.你想创建它们吗?(会使文件体积更大)", "", MessageBoxButtons.YesNo);
            ...
            if (!HasTans)
            {
                DialogResult dialogResult = MessageBox.Show($"网格{Text}没有正方形.你想创建它们吗?(会使文件体积更大)", "", MessageBoxButtons.YesNo);
        ...
        public void Replace(object sender, EventArgs args)
        {
            ...
            OpenFileDialog ofd = new OpenFileDialog();
            ofd.Filter = "支持的格式|*.bfobj;*.fbx;*.dae; *.obj;|" +
             "Bfres Object (shape/vertices) |*.bfobj|" +
             "FBX |*.fbx|" +
             "DAE |*.dae|" +
             "OBJ |*.obj|" +
            ...
            foreach (STBone bone in mdl.Skeleton.bones)
            {
                foreach (string bnam in boneNames)
                {
                    if (bone.Text == bnam)
                    {
                        ...
                        STConsole.WriteLine($"将骨骼添加到形状索引列表! {bone.Text} {index}");
        ...
        public void CreateBoneList(STGenericObject ob, FMDL mdl, bool ForceSkinCount, int ForcedSkinAmount = 4)
        {
            ...
            foreach (var index in mdl.Skeleton.Node_Array) {
                if (index < mdl.Skeleton.bones.Count) {
                    ...
                    {
                        STConsole.WriteLine($"有多块骨头被命名为{bone.Text}.使用第一个.", System.Drawing.Color.Red);
                    }
                    else
                    {
            ...
            foreach (Vertex v in ob.vertices)
            {
                ...
                foreach (string bn in v.boneNames)
                {
                    ...
                    if (hasMatch)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                STConsole.WriteLine($"骨骼{bn}不可蒙皮.此骨骼的顶点将保持未映射状态!", System.Drawing.Color.Red);
                            }
                        }
                    }
                    ...
                    {
                        BonesNotMatched.Add(bn);
                        STConsole.WriteLine($"没有与{bn}匹配的骨骼.此骨骼的顶点将保持未映射状态!", System.Drawing.Color.Red);
            ...
            foreach (Vertex v in vertices)
            {
                for (int i = 0; i < v.boneIds.Count; i++)
                {
                    ...
                    if (isConvertToSmooth)
                    {
                        ...
                        {
                            throw new Exception("转换为平滑:未找到顶点的平滑骨索引" + i.ToString());
                        }
        ...
        public List<ushort> GetIndices(FSKL fskl)
        {
            ...
            foreach (Vertex vtx in vertices)
            {
                foreach (int index in vtx.boneIds)
                {
                    ...
                    {
                        STConsole.WriteLine($"正在保存骨骼索引{bone.Name} {index}");
                        indices.Add(ind);
                    }
                }
            }
        ...
        public Vector3 TransformLocal(Vector3 position, int BoneIndex,bool IsSingleBind,  bool IsPos = true)
        {
            try
            {
                ...
                if (IsSingleBind)
                {
                    ...
                    {
                        STConsole.WriteLine($"将骨绑定到网格的骨索引无效{Text} {BoneIndex} ", System.Drawing.Color.Red);
                        return position;
                    }
                    ...
                    {
                        STConsole.WriteLine($"骨变换的行列式为0以将骨绑定到网格{Text} {BoneIndex} ", System.Drawing.Color.Red);
                        return position;
                    }
                }
                else
            ...
            {
                STConsole.WriteLine("无法将骨骼绑定到网格" + Text, System.Drawing.Color.Red);
                return position;
            }
        }
