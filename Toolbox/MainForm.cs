...
namespace Toolbox
{
    public partial class MainForm : Form, IMdiContainer, IUpdateForm, IMainForm
    {
        ...
        public void OpenFile(string FileName, bool InActiveEditor = false)
        {
            ...
            {
                STConsole.WriteLine($"{FileName} 不支持.");
                return;
            }
        ...
        private void BatchReplaceFTP()
        {
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                foreach (TreeNode node in ObjectEditor.GetNodes())
                {
                    TreeNode foundNode = FindNodeByText(node, "纹理图案动画");
        ...
        private void BatchExportModels(string[] files, string outputFolder)
        {
            ...
            if (failedFiles.Count > 0)
            {
                ...
                STErrorDialog.Show("某些文件无法导出!查看失败文件的详细列表.", "Switch Toolbox", detailList);
            }
            else
        ...
        {
            List<string> Formats = new List<string>();
            Formats.Add("便携式网络图像(.png)");
            Formats.Add("微软DDS(.dds)");
            Formats.Add("联合专家组图像(.jpg)");
            Formats.Add("位图(.bmp)");
            Formats.Add("标签图像文件格式(.tiff)");
